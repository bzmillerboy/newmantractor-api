...
const loopIgArr = async ep => {
  ...
  const iGArr = await Promise.all(
    epImagesSorted.map(async ig => {
      ...
      console.log('This is your asset doc:\n', JSON.stringify(doc._id, null, 2))
      return {
        _type: 'image',
        _key: uuidv4(),
        asset: {
...
const createCategories = async erpData => {
  ...
  const uniqueCategories = data.filter(
    (v, i, a) =>
      a.findIndex(
        t => t.ProductCategoryDesc === v.ProductCategoryDesc && t.EquipmentType === v.EquipmentType
      ) === i
  )

  const dupeCategories = uniqueCategories.map(c => {
    const count = uniqueCategories.filter(obj => obj.ProductCategoryDesc === c.ProductCategoryDesc)
      .length
    if (count > 1) {
  ...
  const changedCategories = categoriesErp.filter(catErp => {
    ...
    const newCategories = () => {
      if (!categoriesSanity.find(catSan => catSan.slug.current === catErp.slug.current)) {
        return catErp
      }
      return null
    }
...
const createLocations = async erpData => {
  ...
  const uniqueLocationsErp = [...new Map(data.map(v => [v.CurrentEquipmentOffice, v])).values()]
  console.log('unique locations from erp data length:', uniqueLocationsErp.length)
  ...
  const changedLocations = locationsErp.filter(locErp => {
    ...
    const newLocations = () => {
      if (!locationsSanity.find(locSan => locSan.slug.current === locErp.slug.current)) {
        return locErp
      }
      return null
    }
...
const createMakes = async erpData => {
  ...
  const makesSanity = await client
    .fetch('*[_type == "equipmentMake" && !(_id in path("drafts.**"))] {_id, _type, name, slug}')
    .then(currMak => {
      return currMak
    })
  ...
  // This reduces the array to unique values to remove duplicate categories in the dataset.
  const uniqueMakesErp = [...new Set(data.map(item => item.ManufacturerDesc))]
  ...
  const changedMakes = makesErp.filter(makErp => {
    ...
    const newMakes = () => {
      if (!makesSanity.find(makSan => makSan.slug.current === makErp.slug.current)) {
        return makErp
      }
      return null
    }
...
const createModels = async erpData => {
  ...
  const currentCategoriesNew = await client
    .fetch('*[_type == "equipmentCategory" && !(_id in path("drafts.**"))] {_id, title, slug}')
    .then(currCat => {
      return currCat
    })
  const currentMakesNew = await client
    .fetch('*[_type == "equipmentMake" && !(_id in path("drafts.**"))] {_id, name, slug}')
    .then(currMak => {
      return currMak
    })
  ...
  const modelsErp = models.map(mod => {
    const slug = slugify(`${mod.ManufacturerDesc}-${mod.Model}--${mod.ProductCategoryDesc}`)
    const currentCategory = currentCategoriesNew.find(
      c => c.slug.current === slugify(mod.ProductCategoryDesc)
    )
    const currentMake = currentMakesNew.find(c => c.slug.current === slugify(mod.ManufacturerDesc))
  ...
  const changedModels = modelsErp.filter(modErp => {
    ...
    const newModels = () => {
      if (!modelsSanity.find(modSan => modSan.slug.current === modErp.slug.current)) {
        return modErp
      }
      return null
    }
...
const createEquipment = async erpData => {
  ...
  const currentModelNew = await client
    .fetch('*[_type == "models" && !(_id in path("drafts.**"))] {_id, name, slug}')
    .then(currMod => {
      return currMod
    })
  const currentLocationNew = await client
    .fetch('*[_type == "location" && !(_id in path("drafts.**"))] {_id, name, slug}')
    .then(currLoc => {
      return currLoc
    })
  ...
  const equipmentErp = data.map(eq => {
    ...
    const currentMake = currentMakesNew.find(c => c.slug.current === slugify(eq.ManufacturerDesc))
    const currentModel = currentModelNew.find(
      c =>
        c.slug.current === slugify(`${eq.ManufacturerDesc}-${eq.Model}--${eq.ProductCategoryDesc}`)
    )
    ...
    const hoursCurrent = eq.CurrentMeterReading1 !== 0 && eq.CurrentMeterReading1
  ...
  const changedEquipment = equipmentErp.filter(eqErp => {
    ...
    const newEquipment = () => {
      if (!equipmentSanity.find(eqSan => eqSan.slug.current === eqErp.slug.current)) {
...
const inventoryProductFetch = async () => {
  const products = await client.fetch(
    '*[_type in ["inventory"] && !(_id in path("drafts.**"))] {_id, price, title, slug, mainImage{asset->{url}}, equipmentCategories->{slug}}'
  )
  return products
}
