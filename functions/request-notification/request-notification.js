...
const {
  SENDGRID_API_KEY,
  SENDGRID_FROM_REQUEST_EMAIL,
  SENDGRID_FROM_REQUEST_NAME,
  SALES_FALLBACK_FL,
  SALES_FALLBACK_KY,
  SALES_FALLBACK,
  RENTAL_FALLBACK
...
exports.handler = Sentry.AWSLambda.wrapHandler(async (event, context, callback) => {
  ...
  const salesContact = () => {
    const salesPersonMatch =
      contact.county &&
      territories.find(c => c.countyName === contact.county && c.state === contact.state)
    if (salesPersonMatch) {
      return salesPersonMatch
    } else if (cartType === 'rental') {
      return JSON.parse(RENTAL_FALLBACK)
    } else if (contact.addressComponents.state && contact.addressComponents.state === 'Florida') {
      return JSON.parse(SALES_FALLBACK_FL)
    } else if (contact.addressComponents.state && contact.addressComponents.state === 'Kentucky') {
  ...
  const notification = {
    ...
    from: {
      email: SENDGRID_FROM_REQUEST_EMAIL,
      name: SENDGRID_FROM_REQUEST_NAME
    },
  ...
  try {
    ...
    // 2) Send data to CRM
    const contactID = await CRMlib.createContact(payload.contact, salesContact().hubSpotOwnerId)
