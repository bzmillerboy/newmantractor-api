...
exports.handler = async (event, context) => {
  ...
  const query = `count(*[_type == 'inventory' && status == "stock"  && !defined(mainImage) && !defined(imageGallery) && (equipmentCategories->categoryType == $type) && (!(deliveryDate > now()) || deliveryDate == "" || deliveryDate == null) && location->slug.current == $location])`;
  ...
  const notification = {
    to: SENDGRID_PHOTO_REPORT_RECIPIENTS.split(","),
    from: {
