...
exports.handler = Sentry.AWSLambda.wrapHandler(async (event, context, callback) => {
  ...
  const replyToEmailValue = fromEmail !== '' ? fromEmail : SENDGRID_FROM_EMAIL
  ...
  try {
    if (email === 'bzmiller82+error@gmail.com') {
      ...
      await hubspot.forms.submit(HUBSPOT_PORTAL_ID, 'd24af600-1186-4270-9560-2398ab01047c', data)
    ...
    return {
      ...
      body: `${e.message || e.body.message} - ${JSON.stringify(e.response.body.errors)}`
    }
  }
})
