...
const equipmentFetch = async (pageNo, pageSize) => {
  ...
  const excludedCategoryId = ["FA", "ZFAS"];
...
const equipmentFetchSold = async (pageNo, pageSize) => {
  ...
  const erpDataFiltered = await erpData.equipmentData.filter((d) => {
    return (
      !excludedCategories.includes(d.ProductCategoryDesc) &&
      (d.EquipmentStatus === "Sold" || d.EquipmentStatus === "Purchase Return")
    );
  });
  ...
  const equipmentErp = data.map((eq) => {
    ...
    const hoursCurrent =
      eq.CurrentMeterReading1 + eq.CurrentMeterReading2 !== 0
        ? Math.max(eq.CurrentMeterReading2 || 0, eq.CurrentMeterReading1 || 0)
        : undefined;
    ...
    const status =
      ...
      (eq.OEMSpecialProgram === "JBR" ||
        eq.OEMSpecialProgram === "KUB" ||
        eq.OEMSpecialProgram === "CAT")
        ? "stock-not-for-sale"
        : "stock";
