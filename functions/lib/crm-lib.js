...
const createDeal = async (payload, salesContact, contactID) => {
  ...
  const { firstName, lastName, startDate, duration, wantDelivery, location } = contact
  const properties = {
    dealname: `${firstName} ${lastName} ${cart[0].subCategoryTitle}`,
    dealstage:
      cartType === 'quote' ? hubSpotApiData.quoteDealstageId : hubSpotApiData.rentalDealstageId,
    hubspot_owner_id: hubSpotProd ? salesContact.hubSpotOwnerId || '91564072' : '183417865', // check env, falls back to stephanie@newmantractor.com,
    closedate: startDate || null,
    rental_duration: duration || '',
    delivery_: wantDelivery,
    job_location: location || '',
    pipeline:
      cartType === 'quote' ? hubSpotApiData.quotePipelineId : hubSpotApiData.rentalPipelineId
  }
  const SimplePublicObjectInput = { properties }

  try {
    const apiResponse = await hubspotClient.crm.deals.basicApi.create(SimplePublicObjectInput)
    console.log('hubspotClient.crm.deals.basicApi.create:', JSON.stringify(apiResponse, null, 2))
...
const getProductIds = async cart => {
  ...
  try {
    const apiResponse = await hubspotClient.crm.products.batchApi.read(batchReadObjectId, archived)
    console.log('hubspotClient.crm.products.batchApi.read:', JSON.stringify(apiResponse.results))
...
const createDealLineItems = async (cart, deal) => {
  ...
  const cartArr = cart.map(async item => {
    ...
    return {
      properties: {
        hs_product_id: producIds.find(product => product?.properties?.cms_id === item._id).id,
...
const createDealLineItemAssociations = async (id, deal) => {
  ...
  try {
    const apiResponse = await hubspotClient.crm.lineItems.associationsApi.create(
      lineItemId,
      toObjectType,
      toObjectId,
      associationType
...
const getAllProducts = async products => {
  ...
  do {
    productBatch = await get100ProductIds(productBatch?.paging?.next?.after || null)
...
const deleteProducts = async (allCmsProducts, allCrmProducts) => {
  ...
  if (batchToDelete.inputs.length > 0) {
    try {
      const apiResponse = await hubspotClient.crm.products.batchApi.archive(batchToDelete)
